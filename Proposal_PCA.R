...
# Identify overlapping years across all sites
long_term_years <- long_term %>%
  group_by(Site) %>%
  summarize(year_range = list(unique(Year))) %>%
  ungroup()

# Find common years across all sites
common_years <- Reduce(intersect, long_term_years$year_range)

# Filter the long-term data to include only the common years
long_term <- long_term %>%
...
all_chemistry <- bind_rows(synoptic, long_term) %>%
...
ggsave("LongTerm_HJA_proposal.png", plot = Fig9, width = 7, height = 7.5, units = "in")

...
# --- 1. Load PT‐ET + compute uncalibrated Hamon ---
data <- read_csv(file.path(input_dir, "PT_ET_methods_timeseries.csv"),
...
site_mean_szilagyi <- monthly_coef_szilagyi %>%
  group_by(SITECODE) %>%
  summarise(coef_szilagyi_site = mean(coef_szilagyi_month), .groups="drop")
...
data <- data %>%
  ...
  mutate(coef_zhang_final  = coalesce(coef_zhang_month,
                                      coef_zhang_site,
                                      global_coef_zhang)) %>%
  left_join(monthly_coef_szilagyi, by = c("SITECODE","MONTH")) %>%
  left_join(site_mean_szilagyi,    by = "SITECODE") %>%
  mutate(coef_szilagyi_final = coalesce(coef_szilagyi_month,
                                        coef_szilagyi_site,
                                        global_coef_szilagyi)) %>%
  mutate(
...
# --- 5. Now redefine your calibration window on the _new_ columns ---
cal_window <- data %>%
  filter(DATE >= as.Date("2013-01-01"),
         DATE <= as.Date("2019-12-31"))
...
reg_sites_zhang    <- cal_window %>%
  filter(!is.na(ET_PT_zhang),
         !is.na(ET_Hamon_pt_zhang_monthly)) %>%
  pull(SITECODE) %>% unique()

reg_sites_szilagyi <- cal_window %>%
  filter(!is.na(ET_PT_szilagyi),
         !is.na(ET_Hamon_pt_szilagyi_monthly)) %>%
  pull(SITECODE) %>% unique()

regressions_zhang <- map(reg_sites_zhang, function(site){
  df <- cal_window %>% filter(SITECODE==site)
  if(nrow(df)>=10) lm(ET_Hamon_pt_zhang_monthly ~ ET_PT_zhang, data=df)
  else NULL
}) %>% set_names(reg_sites_zhang)

regressions_szilagyi <- map(reg_sites_szilagyi, function(site){
  df <- cal_window %>% filter(SITECODE==site)
  if(nrow(df)>=10) lm(ET_Hamon_pt_szilagyi_monthly ~ ET_PT_szilagyi, data=df)
  else NULL
}) %>% set_names(reg_sites_szilagyi)

# (…and then your prediction + final‐fill logic…)

# --- 7. Export out the filled table for Step 3+4 plotting ---
write_csv(data,
          file.path(output_dir, "Hamon_ET_methods_timeseries_monthly_calibrated_filled.csv"))

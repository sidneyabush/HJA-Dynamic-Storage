...
# Step 1: Define helper functions and load the raw data
...
# Step 2: Define the site mapping based on the table
site_mapping <- list(
...
# Step 3: Define the functions for interpolation and site creation

# Extract all required station pairs that need interpolation
extract_pairs <- function(site_mapping) {
  pairs <- list()
  
  for (site in names(site_mapping)) {
    site_info <- site_mapping[[site]]
    
    # Check each variable for pairs
...
# Function to process all pairs and interpolate data
process_pairs <- function(data, pairs, variables) {
  ...
  for (site_name in names(site_mapping)) {
    ...
    for (var_idx in seq_along(variables)) {
      ...
      if (length(stations) == 1) {
        ...
        # Pair of stations - average the interpolated values
        pair_data <- interpolated_data %>%
        ...
        names(pair_data)[2] <- var
        
        if (nrow(site_data) == 0) {
          # First variable, create the dataframe
          site_data <- pair_data %>% mutate(SITECODE = site_name)
        } else {
          # Add variable to existing dataframe
          site_data <- site_data %>% 
            left_join(pair_data, by = "DATE")
        }
      }
    }
...
create_site_summary_plots <- function(site_datasets, site_mapping, variables) {
  for (site_name in names(site_datasets)) {
    ...
    for (var_idx in seq_along(variables)) {
      ...
      subtitle <- if (length(stations) == 1) {
        paste0("Data from ", stations[1])
      } else {
        paste0("Average of ", stations[1], " and ", stations[2])
      }
...
process_meteorological_data <- function(combined_met, site_mapping, variables) {
  ...
  # Extract pairs that need interpolation
  station_pairs <- extract_pairs(site_mapping)
  cat("Station pairs that need interpolation:\n")
  for (pair_name in names(station_pairs)) {
    pair <- station_pairs[[pair_name]]
    cat(sprintf("- %s: %s and %s\n", pair_name, pair$site1, pair$site2))
  }
  
  # Process pairs and interpolate data
  results <- process_pairs(combined_met_clean, station_pairs, variables)
...
# Step 5: Analyze results and write output files
...
write_csv(interpolated_data, file.path(output_dir, "data", "interpolated_met_stations.csv"))
write_csv(all_sites_data, file.path(output_dir, "data", "all_sites_met_data.csv"))

# Write individual site files
for (site_name in names(site_datasets)) {
  write_csv(site_datasets[[site_name]], file.path(output_dir, "data", paste0(site_name, "_met_data.csv")))
}

...
library(colorspace)
library(tidyr)
library(multcompView)   # for compact letter display after Tukey HSD
...
# ─── Instantaneous recession‐limb setup ───────────────────────────────────────
# read drainage area
da_df <- read_csv(
...
# read discharge, filter, compute Q in m3/s
discharge <- read_csv(
...
recession_clean <- discharge %>%
  ...
  mutate(
    dQ             = Q - lag(Q),
    dQ_dt          = dQ / as.numeric(Date - lag(Date)),        # m3/s per day
    recession_slope = -dQ_dt,
    Q_mm_day       = Q / DA_M2 * 86400 * 1000,                  # mm/day
    slope_mm_day   = recession_slope / DA_M2 * 86400 * 1000     # mm/day
  ) %>%
...
# annotation positions for slope
...
# ─── Instantaneous recession‐limb curves (mm/day) ─────────────────────────────
...
for (m in unique(storage_long$metric)) {
  ...
  # 2) Mean ±1 SD summary across years + significance letters
  sum_df <- df %>%
    group_by(site) %>%
    summarise(
  ...
  # compute label positions above the errorbars
...
# ─── Selected‐metrics summary grid with sig. letters + panel labels ──────────
selected_metrics <- c("fdc_slope", "Q5norm", "RBI", "recession_curve_slope")

# prettified names and A–D labels
desc_names <- c(
